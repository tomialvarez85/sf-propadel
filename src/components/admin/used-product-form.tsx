...
import {
  GENERO_VALUES,
  usedProductSchema,
  type UsedProductFormValues,
} from "@/lib/validations/used-product";
...
export function UsedProductForm({
  ...
  mode: "create" | "edit";
  initialData?: UsedProductFormValues & { id: string };
  ...
  const form = useForm<UsedProductFormValues>({
    resolver: zodResolver(usedProductSchema),
  ...
  async function onSubmit(values: UsedProductFormValues) {
    const result =
      mode === "create"
        ? await createUsedProduct(values)
        : await updateUsedProduct(initialData!.id, values);
    ...
    <Form {...form}>
      ...
      >
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Estado de conservación (opcional)</FormLabel>
              <FormControl>
                <Textarea
                  rows={2}
                  placeholder="Ej: Excelente estado, poco uso"
                  {...field}
                  value={field.value ?? ""}
                />
              </FormControl>
        ...
        <div className="flex gap-3">
          ...
          <Button
            type="button"
            variant="outline"
            onClick={() => router.push("/admin/usados")}
          >
            Cancelar
          </Button>
        </div>
