...
import {
  deleteUsedProduct,
  toggleUsedProductActivo,
  updateUsedProductField,
} from "@/app/(admin)/admin/(dashboard)/usados/actions";
import {
  InlineNumberCell,
  InlineSelectCell,
  InlineTextCell,
...
import type { UsedProductListItem } from "@/lib/admin-usados";

export type InlineEditOption = { id: string; nombre: string };

function useDeleteUsedProduct(product: UsedProductListItem) {
  ...
  function handleDelete() {
    startTransition(async () => {
      const result = await deleteUsedProduct(product.id);
...
function UsedProductTableRow({
  product,
  categories,
  brands,
}: {
  product: UsedProductListItem;
  categories: InlineEditOption[];
  brands: InlineEditOption[];
}) {
  const { setConfirmOpen, deleteDialog } = useDeleteUsedProduct(product);
      ...
      <TableRow>
        ...
        <TableCell className="font-medium">
          <InlineTextCell
            ...
            onSave={(value) =>
              updateUsedProductField(product.id, "nombre", value)
            }
        ...
        <TableCell className="text-muted-foreground">
          <InlineSelectCell
            ...
            options={categories}
            onSave={(id) =>
              updateUsedProductField(product.id, "categoryId", id)
            }
        ...
        <TableCell className="text-muted-foreground">
          <InlineSelectCell
            ...
            options={brands}
            onSave={(id) => updateUsedProductField(product.id, "brandId", id)}
        ...
        <TableCell>
          <InlineNumberCell
            ...
            onSave={(n) => updateUsedProductField(product.id, "precio", n)}
        ...
        <TableCell>
          <InlineNumberCell
            ...
            onSave={(n) => updateUsedProductField(product.id, "stock", n)}
        ...
        <TableCell>
          <StatusToggleBadge
            ...
            onToggle={(next) => toggleUsedProductActivo(product.id, next)}
        ...
        <TableCell>
          <div className="flex items-center gap-1">
            <Button asChild variant="ghost" size="icon-sm">
              <Link
                href={`/admin/usados/${product.id}/editar`}
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-muted-foreground flex items-center gap-1 text-xs">
            ...
            <InlineSelectCell
              ...
              options={brands}
              onSave={(id) =>
                updateUsedProductField(product.id, "brandId", id)
              }
              successMessage="Marca actualizada"
              className="w-auto"
            />
...
export function UsedProductList({
  products,
  categories,
  brands,
