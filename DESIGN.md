...
### Hero Banner (home, single fixed image — no longer a carousel)
`HeroBanner` (was `HeroCarousel`) renders exactly one image, sourced from `SiteSettings.heroImagen`/`heroLink`/`heroTitulo` — no embla, no autoplay, no prev/next arrows, no pagination dots. The old `Banner` model (a list with `orden`/`activo`, one row per banner) is gone entirely: the home only ever showed one image at a time, so the list/order/multi-active concept it supported had nothing left to justify it. `/admin/banners` was rewritten to match — a single form (image, título opcional, link opcional), no table, no reorder/activate UI — writing straight to the same `SiteSettings` singleton row every other admin setting already lives on, via its own dedicated Server Action that only ever touches those three columns.
