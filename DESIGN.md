...
- **Images:** all three tiles are fixed local files now (`/public/tiles/hombre.jpg`, `mujer.jpg`, `accesorios.jpg`), hardcoded in `(site)/page.tsx` — **not** admin-editable, at explicit user request. "Hombre"/"Mujer" used to live in two `SiteSettings` fields (`imagenGeneroHombre`/`imagenGeneroMujer`) edited via `/admin/configuracion`; "Accesorios" used to reuse the real Accesorios category's own `Category.imagen`. Both paths are gone entirely now — `Category.imagen` was removed from the schema (categories/marcas lost image upload sitewide, admin included), and the `SiteSettings` hero-tile fields were dropped in the same pass — not just hidden. Changing any of the three images now means replacing the file in `/public/tiles` and redeploying. "Accesorios" still links to the real category (`accesorios?.slug`) and uses its real `nombre` — only the image is now a fixed file, not a live read.
