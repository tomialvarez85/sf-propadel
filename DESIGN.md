...
Replaces the home's former "Visitá nuestro local" map/CTA block (still fully intact at `/contacto`, unchanged — this only removed its home-page duplicate). Curated content: the owner loads testimonials from `/admin/testimonios`, no public submission form — same trust model as `Banner`.
...
- **Carousel mechanics:** same shared `Carousel` (embla) as the hero and Ofertas/Destacados — one carousel implementation, not a new one. Dots (not arrows) for pagination here, wired through `setApi`/`api.on("select")`/`api.scrollTo(index)` — the exact pattern `product-gallery.tsx` already established for its thumbnail strip, reused for dots instead of thumbnails.
