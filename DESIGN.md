...
### Signature texture (implemented)
**The Court-Line Field.** A repeating diagonal-hairline background texture behind all storefront content (public site only, excluded from the admin dashboard). `repeating-linear-gradient(45deg, ...)` — 1px ink lines every 28px, at 12% ink alpha, along the same 45° diagonal as the Angled Corner motif. **Unaffected by the color migration** — always ink, never either accent. Defined as `--pattern-court-lines` in `globals.css`, applied via the `bg-court-lines` utility on the `(site)` layout wrapper only.

### Named Rules
**The Whisper Texture Rule.** Background texture is always ink at ≤14% alpha, never either brand accent. Current value is 12%.
...
### Ofertas / Destacados (homepage lime sections)
Originally solved a real problem found in audit: "Ofertas" and "Destacados" used the identical component with no visual distinction. The fix that stuck, after a few iterations, is the simplest one: both sections use `ProductSection`'s `tone="wash"` — solid `bg-lime`, full-bleed, standard `py-12` padding, no diagonal cut, no accent line — and, as of the latest pass, both also use `layout="carousel"`. The two sections are now visually and structurally identical (title and underlying product data are the only difference); `ProductSection` still supports a `layout="grid"` mode (`grid-cols-2 sm:grid-cols-3 lg:grid-cols-4`, wraps into more rows) as the prop's default, but nothing on the homepage currently uses it. **"Destacados" is the section's title again** — it was briefly renamed "Más Vendidos" partway through this doc's history and later renamed back; the underlying data (`getDestacadoProducts()`, the `destacado` flag) never changed, only the homepage label.
- **`layout="carousel"` mechanics:** reuses the same `Carousel`/`CarouselContent`/`CarouselItem` (embla, `@/components/ui/carousel`) the hero banner already runs on — one carousel implementation in the codebase, not several competing ones (an earlier pass here was a hand-rolled CSS scroll row, replaced once "quiero que sea un carrousel" made the ask explicit). `CarouselItem` overrides the default single-slide `basis-full` with `basis-1/2 sm:basis-1/3 lg:basis-1/4`, showing multiple cards per "page" (matching the old grid's column counts) and sliding by that many at a time via the prev/next buttons, positioned `left-2`/`right-2` — inset within the carousel rather than the component's default `-left-12`/`-right-12`, for the same reason the hero overrides them: those defaults would push the arrows outside a contained (non-full-bleed) width and off-screen on mobile. No autoplay (unlike the hero) — a product carousel the user is expected to browse at their own pace, not an ambient banner.
- **What was tried and removed, in order, so the reasoning isn't lost:** (1) Ofertas alone had a lime wash with a diagonal-cut bottom edge (the Angled Corner motif) and Destacados was plain white — this is what the original audit fix produced. (2) Destacados was renamed "Más Vendidos" and asked to share Ofertas' background color; it first got a 6%-alpha lime tint to literally match the documented value, but that read as barely-there because Ofertas' own "6% fill" was never really 6% in practice (see below) — so it was bumped to solid `bg-lime` to match what Ofertas actually looked like. (3) With both sections now solid lime, Ofertas' diagonal accent line (originally lime, then recolored teal to stay visible against lime on both sides) stopped serving any purpose worth the complexity — a boundary marker between two identical-looking sections isn't marking anything. It was removed outright at explicit user request, along with the `section-highlight-edge`/`section-highlight-fill` utilities that implemented it (deleted from `globals.css`, not left dormant). (4) The section's title was later reverted from "Más Vendidos" back to "Destacados" — none of the color/layout work above depended on the title, so this was a one-line change.
- **Why Ofertas' original "6% wash" never looked like 6%:** its two-layer implementation had a solid `edge` layer sitting *underneath* a translucent `fill`, and `edge` covered more of the section than `fill` did — so translucent-lime-over-opaque-lime (same hue) rendered as just the opaque color. This was only caught once a real screenshot was compared against the source, well after the fact. Worth remembering if a similar "translucent accent over a solid layer" technique gets reused elsewhere: check what's actually *underneath* the translucent layer, not just its own alpha value.
- Both sit under the site-wide Court-Line texture (translucent, so the ink diagonal lines still show through, tinted).
- **Two-Accent Hierarchy Rule tension, worth flagging rather than quietly ignoring:** the rule states lime is "never a large fill." Both sections are now solid lime fills covering full homepage sections — an explicit, repeated user request, not an oversight, but a real departure from that rule worth revisiting if the rule itself is ever reconsidered.
