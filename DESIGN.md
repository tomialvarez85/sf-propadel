...
### Testimonials (home, `bg-primary` — third instance of the inverted-teal surface, not a new treatment)
...
- **Background is `bg-primary` (teal), not ink.** The request framed this as "matching the footer," and the footer's actual fill — confirmed above under Navigation — is solid teal, not ink (`bg-foreground` never appears on the public site; it's admin-login-only, see the Do's/Don'ts ink note). This section, the footer, the Contact page's panel, and Nosotros' mission hero all share one motif: a bounded, deliberate teal-inverted surface for a wayfinding/trust moment, not an energy one. No fourth dark treatment was introduced.
- **Lime dot pagination is the section's one intentional accent moment**, not a violation of "lime is never a large fill" — each dot is a few pixels, the inactive dots are `bg-primary-foreground/30`, only the active one is `bg-lime`. Matches the Two-Accent Hierarchy Rule's own carve-out for "small, bounded secondary moments."
- **Card:** `bg-primary-foreground/5` (a faint lift off the teal field, not a second surface color), a large lime `Quote` glyph (`fill="currentColor"`, decorative — the one place lime fills a shape rather than sitting as a thin accent line, but still small/bounded, not a large-fill violation), the comment in `text-primary-foreground`, and a circular avatar (real photo via the same `ImageUploader`/`cropAspect={1}` pattern as everywhere else, or a `User` icon placeholder) + bold name below.
...
- **Renders nothing when empty** — same rule as the section it replaced: no testimonials loaded (`getActiveTestimonials()` returns `[]`) means the component returns `null`, not an empty dark band.
