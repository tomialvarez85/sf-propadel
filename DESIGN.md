...
- **Lime is a highlight, not a second primary — with one named exception.** Everywhere else it marks urgency/energy in small, bounded doses: the discount/offer badge, and small decorative details (the Angled Corner motif's accent line, the Ofertas section's wash — see Shapes). It never fills a large surface there and never carries body or interactive text, matching how the logo itself uses lime only as a small rim highlight, never as the dominant shape or as lettering. **The header is the deliberate exception**, at explicit user request: it runs a solid `bg-lime` surface, with the wordmark's existing ink/teal split and nav/icon colors unchanged (both read fine against lime's much lighter ground). The footer and Testimonials both stay outside this exception — footer keeps its own solid-teal treatment, Testimonials is back to plain white (see that section) — so this remains a header-only carve-out, not a general license to fill sections with lime.
...
**Full-bleed is a named exception, now used twice.** The hero banner and the home's Category Tiles (see Components) both deliberately break out of `max-w-6xl` to run edge-to-edge (`w-full`, no `mx-auto`/`px-6`) instead of matching the contained width of the sections around them — the hero to read as a wide, immersive banner; the tiles to read as large lifestyle photography rather than a bounded grid. The hero additionally sits flush against the header with no top padding, unlike every other section (including Category Tiles, which keeps normal vertical rhythm between it and its neighbors). A third full-bleed section needs the same bar the second one cleared: a real reason the contained width doesn't serve the content, not decoration for its own sake.

Its height fills exactly what's left of the viewport below the header, identically at every breakpoint, no exceptions: `calc(100dvh - var(--header-height))`. `--header-height` isn't a static number — the header has a conditional promo bar and an extra search row on mobile, so `SiteHeader` measures its own real rendered height with a `ResizeObserver` and writes it to `:root` on mount (`globals.css` only holds a `140px` fallback for the instant before that runs); since it measures the header's actual rendered `offsetHeight`, it's already correct whether mobile's extra search row makes the header taller or shorter than desktop's — no separate mobile measurement path needed. `dvh`, not `vh`: mobile browser chrome (the address bar) shows and hides as the user scrolls, and `vh` is pinned to the *large* viewport regardless, so a hero sized off `vh` reads shorter than the actual visible area whenever the chrome is hidden — `dvh` tracks the real visible viewport at all times, which is what "fill the screen, show nothing of the next section until the user scrolls" actually requires. The Angled Corner's `--hero-corner-cut` (below) moved to `dvh` too, for the same reason — it has to stay in proportion to a hero that's now itself `dvh`-based.
- **History: this used to be capped on mobile, and no longer is.** For a stretch, the formula below `lg` was wrapped in `clamp(360px, …, 620px)` instead of running uncapped — reasoned at the time as avoiding a known mobile anti-pattern (a 100%-remaining-viewport hero burying every other section below the fold on a tall phone). That was superseded by a later, more specific bug: at the capped/aspect-ratio-driven mobile height that replaced it for one pass, the actual uploaded banner image (a wide ~1.8:1 wordmark graphic) got aggressively cropped and zoomed by `object-cover` in the resulting short, wide container — legible on desktop, illegible on mobile. Both concerns are real, but full-viewport height won out at explicit user request: mobile should match desktop's "no peeking at the next section" behavior exactly, with no height reduction carved out for mobile specifically. **The image-crop tradeoff this reopens is real and known, not an oversight** — a portrait-ish full-height mobile container against a landscape banner image means `object-cover` shows a narrower vertical slice of that image than the short/aspect-ratio version did. If a future banner upload looks over-cropped on mobile, the fix belongs in the image (recompose with the important content closer to the vertical center, so a narrow crop still catches it) or in a deliberate follow-up to this rule, not in silently re-adding a mobile height cap — that was tried and explicitly reverted.
...
**The Angled Corner.** A solid-**lime** quarter-disc pie slice sitting in one corner, its curved edge concave into the image (bulging toward the center, not a straight diagonal) — evolved from an earlier straight-clip + thin traced-line version, see history below.
- **The hero banner** (bottom-right corner): an absolutely-positioned SVG (`viewBox="0 0 1 1"`, a single `<path d="M1,0 A1,1 0 0,0 0,1 L1,1 Z">` filled `var(--color-lime)`) sized `18px`/`24px`/`30px`/`40px` across the four breakpoints. The image itself is no longer clipped at all — the shape sits on top of it, not cut into it, so there's no `clip-path` and no `--hero-corner-cut` variable (both removed from `globals.css`).
- **History: used to be a straight `clip-path` diagonal + a separate thin traced lime accent line along the cut**, sized by a `--hero-corner-cut` custom property (`3.5dvh`–`5.5dvh` across breakpoints, itself converted from `vh` to `dvh` when the hero's own height made that same switch — see Layout). Replaced at explicit user request with the single filled curved shape described above; the old variable and utility are gone, not just unused.
...
### Navigation
**Header background is now solid `bg-lime`** (was `bg-background`/white) — the named exception under The Two-Accent Hierarchy Rule, at explicit user request. The header wordmark: **"SFProPadel", one joined word** (was "SF ProPadel" with a space, closed up at explicit user request) — "SF" in ink, "ProPadel" in **teal** (**was orange**), same placement, same Geist Sans display weight (800), same `tracking-[-0.02em]`; teal reads fine against lime's much lighter ground, so the wordmark needed no rework when the surface changed. Nav-link and icon hover states (`hover:text-primary`) stay **teal, not lime**, unaffected by the background swap: lime at 80% lightness still fails text contrast, and teal-on-lime is legible where teal-on-white already was — lime never functions as reading text, on hover or otherwise. The admin-access lock icon stays **`text-muted-foreground`** (neutral gray, unchanged — see Colors): it deliberately sits outside the two-accent system to keep its visual priority low relative to customer-facing icons.

**Favoritos/wishlist — removed entirely**, at explicit user request: the header's heart icon/counter, `WishlistButton`, `useWishlist`, `/favoritos`, and the `heart-pop` keyframe are all gone, not just unused. Product cards and the product detail page no longer have a favorite/wishlist toggle at all.

**Footer (inverted surface, distinct treatment from the header).** The footer's background is a solid **teal** fill (`bg-primary`, was `bg-foreground`/ink) — the one place teal appears as a large surface rather than a small structural accent, matching the request to carry the exact hue of the "ProPadel" wordmark across the whole block. Because the surface itself is now teal, the wordmark here reads as a single flat **"SFProPadel"** (joined, same change as the header) in `text-primary-foreground` (white) rather than the header's two-tone ink/teal split — teal-on-teal would be illegible, and lime can't substitute since it never functions as text (see The Two-Accent Hierarchy Rule). Body copy, nav links, and social icons run at `text-primary-foreground/70`, brightening to full `text-primary-foreground` on hover — not to teal, for the same contrast reason.

**The WhatsApp Float Button (third-party brand exception).** A `fixed` circular button, bottom-right on every public-site page (`(site)/layout.tsx`, not the admin dashboard), opening `wa.me` with `SiteSettings.whatsapp` and a canned greeting. Filled with WhatsApp's own brand green (`#25D366`), not teal or lime — a deliberate, explicit exception to the two-accent system: this specific color is what makes the icon instantly recognizable as "the WhatsApp button" to users, the same reason payment/social brand marks generally keep their own colors rather than being retinted into a host site's palette. Icon is a hand-rolled WhatsApp glyph (`WhatsappIcon`, `components/icons/social.tsx`, same "lucide dropped brand marks" reason Instagram is hand-rolled there) rather than the generic `MessageCircle` lucide icon used for WhatsApp elsewhere in the UI (header, footer, cart) — this is the one prominent, standalone brand-recognition moment where the real logo earns its keep. `shadow-lg` resting shadow and a `motion-safe:hover:scale-105` are the only motion, both gated the same way as every other hover effect in the system. `z-40`: above ordinary page content, but below the `z-50` Sheet/Dialog/Popover/DropdownMenu layer, so it never floats on top of an open cart sheet or admin modal.

### Hero Banner (home, single fixed image — hardcoded, not admin-editable)
`HeroBanner` (was `HeroCarousel`) renders exactly one image — no embla, no autoplay, no prev/next arrows, no pagination dots. Image, título, and link are now literal constants inside `hero-banner.tsx` (`/public/hero/banner-principal.jpg`, "Nueva colección de paletas 2026", `/productos?categoria=paletas`), **not admin-editable at all**, at explicit user request. `/admin/banners` and its Server Action are gone entirely, along with the `SiteSettings.heroImagen`/`heroLink`/`heroTitulo` columns that used to back them (schema migration dropped them) — changing the hero now means replacing the file in `/public/hero` and the three constants, then redeploying. The old `Banner` model (a list with `orden`/`activo`, one row per banner) was already removed in an earlier pass, for the same "the home only ever shows one image" reasoning.
- **Full-bleed sizing, `dvh` height, and the Angled Corner motif are unaffected by this change** — see the Layout and Shapes sections above for those (they describe the current, single-image implementation already).
- Ofertas/Destacados and Testimonials still use the shared `Carousel` component for their own multi-item layouts — removing it from the hero didn't touch those; see their own sections below.

### Product Card (signature component)
Square, `bg-muted`-backed image area with `object-cover` photography, a top-left discount badge (**now lime**, was orange/teal-track), and price/installment copy below (plain ink text, no accent). No wishlist/favorite toggle — removed sitewide, see the Navigation note above. The card's restraint — no border, ring + hover shadow only, exactly one accent-colored element (the badge) — still sets the tone for the rest of the UI.
- **"Usado" badge (`variant="secondary"`, gray)** stacks below the discount badge in the same top-left corner when `Product.condicion === "USADO"` — deliberately a neutral color, not lime or teal, so it never reads as a promotion the way the discount badge does. Same badge repeats on the PDP next to the price, and (when `estadoConservacion` is set) a short line of text below the price, plus a plain `Alert` above the buy buttons warning that used items don't accept returns/exchanges except undisclosed defects.

### Usados (`/usados`, own top-level nav item, fully independent from Productos)
A used-products section, not a filter toggle on `/productos` — its own header nav link (last item, rightmost, both desktop nav and the mobile menu) and its own route. **Deliberately zero shared listing/query code with `/productos`**, at explicit user request, after a bug (an unrelated bad test-image host) got misdiagnosed as coupling between the two: `used-product-query.ts` (own `getUsedProductListing`/`getUsedFilterFacetCounts`, hard-coded `condicion: "USADO"`) and `used-listing.tsx` (own top-level orchestrator) are separate files from `product-query.ts`/`product-listing.tsx`, which in turn hard-codes `condicion: "NUEVO"` in its own `where` — the same baseline-scope status as `activo: true`, not a removable filter. A product's condición is exclusive: it's either in `/productos` or `/usados`, never both. The generic filter/sort/pagination UI (`ProductFilters`, `ActiveFilters`, `ProductSort`, `ProductPagination`) IS still shared between the two — they carry zero condición-specific logic (categoría/marca/género/precio/orden/page only), so duplicating them would be copy-pasted dead weight rather than removing any real coupling. Same split exists in the admin: `/admin/usados` has its own page/list/filters/form/actions/query layer (`admin-usados.ts`, `usados/actions.ts`, `used-product-*.tsx`), independent from `/admin/productos`'s equivalents — condición is implicit and not a form field in either (NUEVO by the DB default, USADO hard-coded server-side in the usados actions), rather than a user-editable select.
- **`UsedProductCard`** (site-facing grid) is its own component too, not `ProductCard` with an optional prop — visibly different from the standard card (adds a `line-clamp-3` description pulled from `descripcion`, omitted entirely rather than left blank when empty) and consumes its own `UsedProductCardData` type from `used-product-query.ts`, not the shared `ProductCardData`.

### Ofertas / Destacados (homepage sections — lime wash retired)
Originally solved a real problem found in audit: "Ofertas" and "Destacados" used the identical component with no visual distinction. For a long stretch the fix was both sections sharing `ProductSection`'s `tone="wash"` — solid `bg-lime`, full-bleed — plus, in the last pass before this one, both on `layout="carousel"`. **The `tone`/`wash` mechanism itself is now retired**, at explicit user request, as part of the same broader decision that removed the Court-Line texture: every content background on the site, public and admin, is flat solid white now, with no section-level tint anywhere except the header (now lima) and the footer's inverted teal block. `ProductSection` no longer takes a `tone` prop — there was exactly one caller of `tone="wash"` (this component, both instances) and once it went away the prop had nothing left to do, so it was deleted rather than kept as a dormant option. Both sections are now plain white, `layout="carousel"`, structurally identical (title and underlying product data are the only difference). **"Destacados" is the section's title again** — it was briefly renamed "Más Vendidos" partway through this doc's history and later renamed back; the underlying data (`getDestacadoProducts()`, the `destacado` flag) never changed, only the homepage label.
- **`layout="carousel"` mechanics (unaffected by the background change):** the shared `Carousel`/`CarouselContent`/`CarouselItem` (embla, `@/components/ui/carousel`) — the same one Testimonials runs on, one carousel implementation in the codebase, not several competing ones. `CarouselItem` overrides the default single-slide `basis-full` with `basis-1/2 sm:basis-1/3 lg:basis-1/4`, showing multiple cards per "page" and sliding by that many at a time. No autoplay — a product carousel the user is expected to browse at their own pace, not an ambient banner. (The hero banner used to run this same carousel too, with autoplay, until it was simplified to a single fixed image — see Layout and the hero's own section under Components.)
...
Three large, equal-width photography tiles — "Hombre," "Mujer," "Accesorios" — replacing the home's previous 5-item `rounded-xl` category grid (`category-grid.tsx`, plus the now-unused `getMainCategories()` that fed it — both deleted outright once `Category.imagen` was removed from the schema, rather than left as dead code that could no longer compile). "Hombre"/"Mujer" link to `/productos?genero=hombre`/`mujer` (the género filter, not a real category); "Accesorios" links to the real Accesorios category.
...
- **Images:** "Hombre"/"Mujer" are fixed local files (`/public/tiles/hombre.jpg`, `/public/tiles/mujer.jpg`), hardcoded in `(site)/page.tsx` — **not** admin-editable, at explicit user request. They used to live in two `SiteSettings` fields (`imagenGeneroHombre`/`imagenGeneroMujer`) edited via `/admin/configuracion`; those fields, their form section ("Tiles de género (home)"), and their `ImageUploader` controls are gone entirely (schema migration dropped the columns), not just hidden. Changing either image now means replacing the file in `/public/tiles` and redeploying. **"Accesorios" has no image at all now** — `Category.imagen` was removed from the schema entirely (categories/marcas lost image upload sitewide, admin included), so this tile always falls through to `CategoryTiles`' existing `ImagePlaceholder` branch. This is a real visual regression versus the photo it used to show, accepted as a side effect of removing category images everywhere, not something separately hardcoded like the other two tiles.

### Testimonials (home, white section, teal cards matching the footer)
Replaces the home's former "Visitá nuestro local" map/CTA block (still fully intact at `/contacto`, unchanged — this only removed its home-page duplicate). Curated content: the owner loads testimonials from `/admin/testimonios`, no public submission form — admin-only content, not user-generated.
- **The section itself has no background** (plain white, same "every content background is flat white" rule as Ofertas/Destacados) — **only the individual cards are solid `bg-primary`/teal**, matching the footer's own fill, at explicit user request. This is a card-level echo of the footer's inverted surface, not a section-level one: white page, teal islands.
- **Card:** `bg-primary`/`text-primary-foreground` (the footer's own token pair), a lima `Quote` glyph (`fill="currentColor"`, small decorative accent — the original teal-card/lima-accent pairing this section started with), comment text in `text-primary-foreground`, and a circular avatar (real photo via the same `ImageUploader`/`cropAspect={1}` pattern as everywhere else, or a `User` icon placeholder in `text-primary-foreground/50`) + bold name in `text-primary-foreground` below.
- **Dot pagination:** inactive `bg-muted-foreground/30`, active `bg-lime` — a small, bounded accent dot, same reading as the discount badge under The Two-Accent Hierarchy Rule.
- **Carousel mechanics:** same shared `Carousel` (embla) as Ofertas/Destacados — one carousel implementation, not a new one (the hero banner used to run it too, before being simplified to a single fixed image — see its own section under Components). Dots (not arrows) for pagination here, wired through `setApi`/`api.on("select")`/`api.scrollTo(index)` — the exact pattern `product-gallery.tsx` already established for its thumbnail strip, reused for dots instead of thumbnails.
- **Renders nothing when empty** — same rule as the section it replaced: no testimonials loaded (`getActiveTestimonials()` returns `[]`) means the component returns `null`.

### Contact Page (`/contacto`)
**Simplified to a single centered column**, at explicit user request — the "Contacto directo" panel (WhatsApp/Teléfono/Email/Dirección list + embedded map) was removed entirely, along with the two-column `lg:grid-cols-5` layout that used to hold it. The page is now just the "Envianos un mensaje" form, `max-w-2xl`, centered, `py-16`. Submitting it builds a `wa.me` message from the form fields (same pattern as the product-detail page and the floating WhatsApp button) instead of posting anywhere — there's no backend for this form. **This page uses no lime at all** — there's no energy/decorative moment to hang it on here, and forcing one in just to use the color somewhere would be decoration for its own sake (see The Angled Corner Family Rule).
- **Form card:** plain `Card`/`CardContent` (`rounded-xl`, `bg-card`, `ring-foreground/10`, no shadow) — reads as elevated through contrast with the page (white card, hairline ring) rather than literally adding a shadow, per The One Lifted Element Rule under Elevation & Depth.
- **The map and the WhatsApp/Teléfono/Email/Dirección list are gone from this page**, not just restyled — `LocationMap` (the component that rendered the embedded map here) is now unused sitewide, deliberately kept undeleted in case it's wanted somewhere else later (the home's "Visitá nuestro local" section, which used to be its other caller, was itself replaced by Testimonials — see that section).
- Inputs are untouched — `Input`/`Textarea` already carry `rounded-lg`, `h-8`, teal `focus-visible:ring` globally.

**Historical note — the lime-background bug that used to motivate this page's design, kept for context.** An earlier pass had this page (and Nosotros, and the product listing) on a full-bleed lime background, discovered from a screenshot rather than the source — a direct violation of "lime is never a large fill, never a text/border color." All three were independent copy-paste of the same `bg-lime min-h-full` mistake, not a shared component; all three were fixed, confirmed via a full `src/` grep for `bg-lime` at the time. That investigation no longer describes this page's current implementation (see above), but the reasoning is left here so nobody reintroduces a lime page background from an old memory of this doc.
...
- **Do** use lime only for small, bounded secondary moments: the discount badge, and small decorative details (the hero's Angled Corner accent line) — never as text. (The header is a named, explicit large-fill exception — see the callout under The Two-Accent Hierarchy Rule. Ofertas/Destacados's lime wash was a similar earlier exception but has since been retired — see that section.)
...
- **Don't** use lime as a text color anywhere. **Don't** use lime as a large fill outside the header — that's the one named exception (see The Two-Accent Hierarchy Rule); everywhere else it's still a hard limit from the brand's own logo usage.
